...
def summarize_news_article(url: str) -> str:
    ...
    try:
        ...
        main_content = soup.find('main') or soup.find('article') \
            or soup.find('div', class_='content') or soup.find('div', id='content')
        if main_content:
        ...
        article_text = ' '.join(chunk for chunk in chunks if chunk and len(chunk) > 20)[:2000]  # Limit length
...
def get_market_sentiment() -> Dict[str, Any]:
    ...
    try:
        ...
        news_text = "\n".join([f"- {article['title']}: {article['description']}" for article in news[:5]])
        ...
        return {"overall": "neutral", "confidence": 0.5, "key_themes": [], "error": str(e)}
...
def ingest_news_to_vectorstore(query: str = "investments OR stocks OR bonds OR markets", days_back: int = 1):
    ...
    for article in news:
        if article['url'] and article['title']:
            ...
            try:
                ingest_document(temp_file, metadata)
