...
def ingest_md_document(
    ...
    return len(chunks), time_taken


def ingest_directory(directory_path: str) -> tuple[int, float]:
    ...
    for md_file in path.glob("*.md"):
        docs_inserted, time_taken = ingest_md_document(md_file)
        total_docs_inserted += docs_inserted
        total_time_taken += time_taken

    return total_docs_inserted, total_time_taken


def ingest_urls(urls: list[str]) -> tuple[int, float]:
    ...
    for url in urls:
        ...
        try:
            docs_inserted, time_taken = ingest_md_document(tmp_file_path, metadata={"url": url})
            total_docs_inserted += docs_inserted
            total_time_taken += time_taken
        except Exception as e:
...
if __name__ == "__main__":
    ...
    total_docs_inserted, total_time_taken = ingest_urls(urls)
    print(
        f"Ingested total {total_docs_inserted} chunks "
        f" in {total_time_taken:.2f} seconds."
    )
